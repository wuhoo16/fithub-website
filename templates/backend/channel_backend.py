...
class ChannelBackend(ModelInterface, Channel):
    ...
    def get_related_objects_for_instance(id, db):
        ...
        relatedExercises = ModelInterface.find_related_instances_based_on_subcategory(subcategory, db.exercises, ['category', category], ['subcategory', subcategory], ModifiedInterface.EXERCISES_ARRAY)
        ...
        relatedEquipments = ModelInterface.find_related_instances(db.equipments.find({'equipmentCategory': equipmentCategory}), ModifiedInterface.EQUIPMENT_ARRAY)
        relatedChannels = ModelInterface.find_related_instances_based_on_subcategory(subcategory, db.channels, ['exerciseCategory', category], ['exerciseSubcategory', subcategory], ModifiedInterface.CHANNEL_ARRAY)
    ...
    def filter(db, requestForm):
        ...
        for subscriberRangeString in selectedSubscriberRange:
            ...
            filteredChannels = np.array(ModelInterface.find_related_objects(db.channels.find({'subscriberCount': {'$gte': int(subscriberRangeList[0]), '$lt': int(subscriberRangeList[1])}}), ModifiedInterface.CHANNEL_ARRAY))
        ...
        for totalViewsString in selectedTotalViewsRange:
            ...
            filteredChannels = np.append(filteredChannels, np.array(ModelInterface.find_related_objects(db.channels.find({'viewCount': {'$gte': int(totalViewsList[0]), '$lt': int(totalViewsList[1])}}), ModifiedInterface.CHANNEL_ARRAY)))
        ...
        for videoRangeString in selectedVideosRange:
            ...
            filteredChannels = np.append(filteredChannels, np.array(ModelInterface.find_related_objects(db.channels.find({'videoCount': {'$gte': int(videoRangeList[0]), '$lt': int(videoRangeList[1])}}), CHANNEL_ARRAY)))
