...
class ExerciseBackend(self, Exercise):
    ...
    def initialize_array_from_mongo_database(db):
        ...
        self.EXERCISES_ARRAY = exercise_array

    @staticmethod
    def get_related_objects_for_instance(id, db):
        attributes = self.find_current_instance_object(id, db.exercises, ('category', 'subcategory', 'equipment'))
        ...
        for equipmentCategory in equipmentCategoryList:
            relatedEquipments = np.append(np.array(relatedEquipments), np.array(self.find_related_objects(db.equipments.find({'equipmentCategory': equipmentCategory}), self.EQUIPMENT_ARRAY)))
    ...
    def filter(db, requestForm):
        ...
        for exerciseCategory in selectedExerciseCategories:
            filteredExercises = np.array(self.find_related_objects(db.exercises.find({'category': exerciseCategory}), self.EXERCISES_ARRAY))
        ...
        for equipmentCategory in selectedEquipmentCategories:
            filteredExercises = np.append(filteredExercises, np.array(self.find_related_objects(db.exercises.find({'equipment': equipmentCategory}), self.EXERCISES_ARRAY)))
